var GLOBAL_VAR = {
    ...
    //url: http://192.168.5.112/trade/kline?symbol=BTC_USDT&range=60000&limit=1000&prevTradeTime=1537338310573
};
...
Chart.prototype.setTitle = function () {
    ...
    var a = coinkline = getCookie("tradcoinname") + "_" + getCookie("maincoinname");
    a += " ";
            ...
            if (GLOBAL_VAR.G_HTTP_REQUEST) {
                ...
                try {
                    if (!GLOBAL_VAR.chartMgr.updateData("frame0.k0", GLOBAL_VAR.KLineData)) {
                        ...
                        //console.log("解析成功...");
                        //推送点下次请求
                        // GLOBAL_VAR.TimeOutId = setTimeout(RequestData, 30*1000);//可用
                        return
                    } else {
                        //console.log("解析失败...");
                    }
...
function setHttpRequestParam(b, c, a, e) {
    ...
    var d = "needTickers=1&symbol=" + coinkline + "&type=" + c;
    if (a != null) {
        d += "&size=" + a
    } else {
        d += "&since=" + e
