<template>
  <div class="app-main wh-full">
    ...
    <div
      v-if="address"
      class="content van-hairline--top app-body h-full"
    >
    ...
    <div
      v-else
      class="h-full flex flex-v-center flex-h-center font-16 color-999"
    >暂无数据</div>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    getRechargeAddr(coin).then(res => {
      if (res.address[0]) this.address = res.address[0].address;
    });
  },
  methods: {
