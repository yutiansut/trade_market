<template>
    <div class="content">
        <div class="title font-16 font-bold">账户安全</div>
        <!-- 认证 -->
        <div class="table-wrap bd-bottom">
            <div class="header">安全认证</div>
            <ul class="safe-auth">
              <li v-for="(item,key) in authConfig" :key='key'>
                <span><i class="el-icon-warning color-danger"></i>&nbsp;&nbsp;&nbsp;{{item.label}}</span>
                <span v-text="item.description" class="txt-center color-999"></span>
                <span v-if='!item.isBind' class="txt-rt">
                  <a @click="bindHandle(key)" class="color-danger" href="javascript:" v-text="item.stateLabel"></a>
                </span>
        ...
        <div class="table-wrap bd-bottom-none mt-20">
            <div class="header">安全日志</div>
            <el-table :data='authLogs'>
              <el-table-column width='250' label='序号' type='index'></el-table-column>
        ...
        <div class="table-wrap bd-bottom-none mt-20">
            ...
            <el-table :data='loginLogs'>
              ...
              <el-table-column width='100' label='类型' prop='type'></el-table-column>
            </el-table>
        </div>
        <!-- 绑定登录密码 -->
        <bind-password 
