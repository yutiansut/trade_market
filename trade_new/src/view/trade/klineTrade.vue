            ...
            <div slot="top-slot" class="k-line-top">
                    ...
                    <div class="table p-abs">
                        <div class="thead">
                            <span
                                ...
                                v-for="(item,v) in mainCoinList.maincoin"
                                :class="mainCoinId==item.coinid?'active':''"
                                :key='v'
                                v-text="item.coinid">
                            </span>
                            <span @click="getCustomList"
                                :class="mainCoinId=='opt'?'active':''"
                                v-text='$t("optional")'>
                            </span>
                        </div>
                        <div class="table-content">
                            <div
                                class="tr flex flex-between">
                            ...
                            <div v-loading='showLoading'
                                ...
                                @click='onTableRowClick(item)'
                                    v-for="(item,index) in tableData" :key='item.autoid'>
                                    <div @click="addMylist(item,index)" class="td">
                                        <i :class="item.isMyLike?'el-icon-star-on':'el-icon-star-off'"
                                        class="font-16"></i>
                                    </div>
                                    ...
                                    <div class="td">
                                        <span
                                            :class="item.rise*1>0?'color-danger':'color-success'">
                                            {{item.rise*1}}
                                        </span>                                        
                                    </div>
                                    <div class="td">
                ...
                <div class="price flex fl">
                    <div class="item h-full">
                        <em class="font-16 color-success font-bold">{{latestData.prise*1}}</em>&nbsp;
                        <em class="price-1 font-12">≈&nbsp;￥{{latestData.cny*1}}</em>
                    </div>
                    <i class="tag">|</i>
                    <div class="item h-full fl">
                        <em class="label" v-text="$t('increase')"></em>
                        <em
                            :class="currentConInfo.rise>0?'color-danger':'color-success'"
                            class="price-1 font-14">&nbsp;{{latestData.rise*1}}%
                        </em>
                    </div>
                    <i class="tag">|</i>
                    <div class="item h-full fl">
                        <em class="label" v-text="$t('high')"></em>
                        <em class="price-1 font-14">{{latestData.height*1}}</em>
                    </div>
                    <i class="tag">|</i>
                    <div class="item h-full fl">
                        <em class="label" v-text="$t('low')"></em>
                        <em class="price-1 font-14">{{latestData.low*1}}</em>
                    </div>
                    <i class="tag">|</i>
                    <div class="item h-full fl">
                        <em class="label" v-text="$t('dayVol')"></em>
                        <em class="price-1 font-14">{{latestData.number*1}}</em>
                    </div>
                </div>
            </div>
        </my-header>
                ...
                <div class="trade-panel flex flex-between">
                    <div class="trade-table">
                        ...
                        <ul v-if="latestSoldData.length>0" class="tb-left trade-body">
                            <li 
                                @click='onLatestLiClick(item,1)'
                                v-for="(item,index) in latestSoldData"
                                :key='index'
                                ...
                                <div :style='{width:(item.total/sellListTotal)*100+"%"}' class="p-abs progress"></div>
                            </li>
                        </ul>
                        <div v-else class="table-no-data txt-center" v-text="$t('label108')"></div>
                        <!-- 最新价 -->
                        <div v-show="latestData.prise*1" class="newprice txt-center color-danger">
                            <span class="font-16">
                                {{latestData.prise*1}}
                            </span>
                            <em>≈￥{{latestData.cny*1}}</em>
                        </div>
                        <!-- 卖出五档图 -->
                        <ul v-if="latestBuyData.length>0" class="tb-left trade-body">
                            <li
                                @click='onLatestLiClick(item,2)'
                                v-for="(item,index) in latestBuyData"
                                :key='index'
                                class="flex flex-between p-rel">
                                <span class="color-success tr-mPrice">{{$t('buy')}}</span>
                                <span class="tr-name" v-text="item.price*1"></span>
                                <span class="tr-vol" v-text="item.number*1"></span>
                                <div :style='{width:(item.total/buyListTotal)*100+"%"}' class="p-abs progress"></div>
                            </li>
                        </ul>
                        <div v-else class="table-no-data txt-center" v-text="$t('label108')"></div>
                    </div>
                    <div class="trade-table">
                        ...
                        <ul v-if="historicalBuyData.length>0" class="tb-right trade-body">
                            <li
                                v-for="(item,index) in historicalBuyData"
                                :key='index'
                                class="flex flex-between p-rel">
            ...
            <div class="bottom">
                <div class="record">
                    ...
                    <template v-if="userData.isLogin">
                        ...
                        <div
                            element-loading-background='rgba(0,0,0,.4)'
                            v-loading='showLoading_1' v-show="contentId==0"
                            class="entr-content">
                        ...
                        <div
                            ...
                            v-show="contentId==1" class="entr-content">
                            <div class="flex entr-head flex-between">
                                <div style="flex:1;" v-text="$t('time')"></div>
                                ...
                                <div style="flex:1.5;" v-text="$t('commision')"></div>
                                <div style="flex:0.5;" v-text="$t('type')"></div>
                            </div>
                            <div class="entr-body">
                                <div class="row flex flex-between"
                                    v-if='myHistoryEntrustList.length>0'
                                    v-for="(item,index) in myHistoryEntrustList"
                                    :key='index'>
                                    <div style="flex:1" v-text="item.writedate"></div>
                                    <div style="flex:1.5;" v-text="item.tradcoin"></div>
                                    <div style="flex:1.5;" v-text="item.price*1"></div>
                                    <div style="flex:1.5;" v-text="item.number*item.price"></div>
                                    <div style="flex:1.5;" v-text="item.dealnumber*item.price"></div>
                                    <div style="flex:1.5;" v-text="item.tradgas"></div>
                                    <div style="flex:0.5;"
                                        :class="item.state=='0'?'color-danger':'color-success'"
                                        v-text="item.state=='0'?$t('buy'):$t('sell')">
                                    </div>
                                </div>
                                <div v-if='myHistoryEntrustList.length==0' v-text="$t('label108')"></div>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="transaction-content">
                        ...
                        <div class="form transaction-sell">
                            ...
                            <p class="transaction-input transaction-xj">
                                <input id="getCountPrice" class="tread-input"
                                :placeholder="$t('buyingRate')"
